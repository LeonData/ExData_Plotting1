#
# This R script file implements task 4 for Project 1
#
...
if( nrow(d) > 2 )
{
  # We are going to plot 4 plots (in a matrix 2x2) using slighlty increased margins
  ...
  with(d, plot(d$Global_active_power,type="l",
               ylab="Global active power",xlab="", axes=FALSE, frame.plot=TRUE ) )
  # Add default Y axis
  axis(2)
  ...
  plot(d$Voltage,type="l",
       ylab="Voltage",xlab="datatime", axes=FALSE, frame.plot=TRUE )
  # Add default Y axis
  axis(2)
  # Add altered X acis - display days of the week
  axis(1, at=xticks, labels=xlabels) 
  ...
  plot(d$Sub_metering_1,type="l",
               ylab="Energy sub metering",xlab="", axes=FALSE, frame.plot=TRUE )
  # Adding d$Sub_metering_2
  lines(d$Sub_metering_2,col="red")
  # Adding d$Sub_metering_3
  lines(d$Sub_metering_3,col="blue")
  # Add legend at the right top corner without a border
  ...
  plot(d$Global_reactive_power,type="l",
       ylab="Global_reactive_power",xlab="datatime", axes=FALSE, frame.plot=TRUE) 
  ...
  # Return global parameters back to their defualt values
